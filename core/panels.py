...
class ReadOnlyPanel(EditHandler):
    """ ReadOnlyPanel EditHandler Class -
        built from ideas on https://github.com/wagtail/wagtail/issues/2893
        Most credit to @BertrandBordage for this.
        Usage:
        attr:               name of field to display
        style:              optional, any valid style string
        add_hidden_input:   optional, add a hidden input field
                            to allow retrieving data in form_clean (self.data['field'])
        If the field name is invalid, or an error is received getting the value,
        empty string is returned.
        """

    def __init__(self, attr, style=None, add_hidden_input=False,
    ...
    def hidden_input(self):
        # add a hidden input field if selected,
        # field value can be retrieved in form_clean with self.data['field']
    ...
    def heading_tag(self, tag):
        ...
        if not self.heading:
            ...
            result = format_html('<legend>{}</legend>', self.heading)
        else:
            result = format_html('<label>{}{}</label>', self.heading, ':')
...
class CodePanel(ReadOnlyPanel):
    def render(self):
        # return formatted field value
        self.value = self.get_value()
        return format_html(
            '<div style="padding-top: 1.2em;"><pre><code style="display: inline-block">{}</code></pre></div>',
            self.value,
        )


class JSONPanel(ReadOnlyPanel):
    def render(self):
        # return formatted field value
        self.value = self.get_value()
        return format_html(
            '<div style="padding-top: 1.2em;"><pre><code style="display: inline-block">{}</code></pre></div>',
            json.dumps(self.value, indent=4, ensure_ascii=False),
        )
...
class RichHelpPanel(EditHandler):
    ...
    def get_value(self, field_name):
        # if field_name is a valid name, return data,
        # otherwise return field name unchanged
            ...
            for item in self.value_dict:
                try:
                    parsed_text = parsed_text.replace(
                        '{{' + item + '}}',
                        self.get_value(str(self.value_dict[item])),
                    )
                except Exception:
                    pass
